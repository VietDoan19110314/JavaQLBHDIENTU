...
public class QLCuahangDienTu {
	...
	public static void Cau1() {
		var Sanphamtren10trieus = dsSanPham
				.stream()
				.filter(c->c.getDonGia() > 10000000)
				.collect(Collectors.toList());
		for (SanPham Sanphamtren10trieu : Sanphamtren10trieus) {
			System.out.println(Sanphamtren10trieu.getTenSanPham());
		}
	}
	
	public static void Cau2() {
		List<Date> lsNgaySinh = dsNhanVien.stream().map(NhanVien::getNgaysinh).sorted().collect(Collectors.toList());
		Date ngaySinhNhoNhat = lsNgaySinh.get(0);
		List<NhanVien> NhanVienLonTuoiNhat = dsNhanVien.stream()
